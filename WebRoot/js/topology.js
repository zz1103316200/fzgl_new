...
//点击服务图标后的弹框里的服务访问量的曲线喂数据
function showAreaGraph2(serviceVisitTimes,timeStr){
	var data = new Array();
...
function showBarChar2(serviceRunTime,ordTime){
	...
	for (var i = 0; i < 11; i += 1){
		//alert(ordTime[i]);
		if(i%2==0){
			$("#bar-chart div.tickLabels div.xAxis div.tickLabel").eq(i).html(ordTime[i/2]);
		}else{
			$("#bar-chart div.tickLabels div.xAxis div.tickLabel").eq(i).html(" ");
		}
             ...
             var service = d.name.split("_");
				...
				alert("serviceid:" + serviceid);
				  ...
				  $.post("./newServlet/yxjk_3_3_12_fwxq_Servlet",{serviceID:serviceid},function(text){
	     				//alert("sdsd");
	     				//寰楀埌鍥惧舰BarChar
					 // alert("diaoyong:" + text.SingleServiceRunTime);
					  showBarChar2(text.SingleServiceRunTime,text.ordTime1);
	     				...
	     				showAreaGraph2(text.SingleServiceCallQuantity,text.ordTime);
	     				//鏄剧ずxy杞寸殑鍧愭爣
